  ...
  const fetchBoxes = async () => {
    try {
      ...
      setBoxes([
        ...
        { _id: '2', name: 'صندوق الفضي الماسي', description: 'جوائز قيمة', rarity: 'epic', icon: '💎', cost: 500, prizes: [{},{},{}] },
        ...
        { _id: '5', name: 'صندوق الزمرد', description: 'مكاسب مضمونة', rarity: 'rare', icon: '💚', cost: 300, prizes: [{},{},{}] },
  ...
  return (
    <div className="premium-bg min-h-screen p-6 pt-24 pb-32">
      ...
      <header className="flex justify-between items-center mb-12" style={{ position: 'relative' }}>
        <div>
          <h1 className="text-5xl font-bold mb-2"><span className="shimmer-text">الصناديق</span></h1>
      ...
      <div className="glass-premium p-6 mb-12 glow-pulse" style={{ position: 'relative' }}>
        <div className="flex items-center justify-between">
          ...
          <Link to="/shop" onClick={onClick} className="btn-3d btn-premium">تسوق واكسب نقاط</Link>
        </div>
      </div>
        ...
        {['legendary', 'epic', 'rare', 'uncommon'].map(rarity => (
          <div key={rarity} className="flex items-center gap-2">
            ...
            <span className={getRarityColor(rarity)}>
              {rarity === 'legendary' ? '👑 أسطوري' : rarity === 'epic' ? '💜 ملحمي' : rarity === 'rare' ? '💎 نادر' : '🟢 شائع'}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-8">
        {boxes.map((box, index) => (
          <div key={box._id || index} className="card-3d category-card fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
            <div className="glass-premium rounded-2xl overflow-hidden">
              <div className="relative h-64 flex items-center justify-center holographic">
                ...
                {box.discount > 0 && (
                  <div className="absolute top-4 right-4 animated-border">
                    <div className="px-4 py-2 rounded-full bg-red-500 text-white font-bold animate-pulse">-{box.discount}%</div>
                  </div>
                )}
              </div>
              ...
              <div className="p-6">
                <div className={`text-sm font-bold mb-2 ${getRarityColor(box.rarity)}`}>
                  {box.rarity === 'legendary' ? '👑 أسطوري' : box.rarity === 'epic' ? '💜 ملحمي' : box.rarity === 'rare' ? '💎 نادر' : '🟢 شائع'}
                </div>
                ...
                <div className="flex gap-2 mb-6 justify-center">
                  {[1,2,3,4].map((_, i) => (
                    <div key={i} className={`w-14 h-14 rounded-xl flex items-center justify-center bg-gradient-to-br ${getRarityGradient(box.rarity)} float-3d`} style={{ animationDelay: `${i * 0.2}s` }}>
                      <span className="text-2xl">{box.rarity === 'legendary' ? '👑' : box.rarity === 'epic' ? '💜' : box.rarity === 'rare' ? '💎' : '🎁'}</span>
                    </div>
                  ))}
                </div>
      ...
      {opening && (
        <div className="modal-premium">
          <div className="text-center">
            ...
            <div className="loading-dots mt-4"><span></span><span></span><span></span></div>
          </div>
        </div>
      )}
        ...
        <div className="modal-premium">
          <div className="modal-content holographic">
            ...
            <div className={`bg-gradient-to-br ${getRarityGradient(result.prize?.rarity)} rounded-2xl p-8 mb-6`}>
              <div className="text-7xl mb-4 float-3d">{result.prize?.rarity === 'legendary' ? '👑' : '🎁'}</div>
            ...
            <button onClick={() => { setResult(null); onClick(); }} className="btn-3d btn-premium w-full text-xl">إغلاق</button>
          </div>
        </div>
      )}
    </div>
