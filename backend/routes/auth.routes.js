...
router.post('/register',
  [body('username').isLength({ min: 3, max: 30 }), body('password').isLength({ min: 6 }), body('name').notEmpty()],
  validate,
  async (req, res) => {
    try {
      const { username, password, name, email, referralCode } = req.body;
      const existingUser = await User.findOne({ $or: [{ username }, { email }] });
      if (existingUser) return res.status(400).json({ success: false, error: 'المستخدم موجود مسبقاً' });
...
router.post('/login',
  ...
  async (req, res) => {
    try {
      ...
      if (!isMatch) return res.status(401).json({ success: false, error: 'بيانات الدخول غير صحيحة' });
...
router.put('/profile', authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.user.id, { name, email, avatar, phone, preferences }, { new: true }).select('-password');
    res.json({ success: true, data: user });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
router.put('/password', authMiddleware,
  ...
  async (req, res) => {
    try {
      ...
      if (!isMatch) return res.status(400).json({ success: false, error: 'كلمة المرور الحالية غير صحيحة' });
