...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const apiKey = await ApiKey.create({
      user: req.user.id, name, key, prefix,
      permissions: permissions || ['read'],
      rateLimit: rateLimit || 100, expiresAt
    });

    res.status(201).json({ success: true, data: { ...apiKey.toObject(), fullKey: key } });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if (!apiKey) return res.status(404).json({ success: false, error: 'المفتاح غير موجود' });
